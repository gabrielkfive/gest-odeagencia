...
type Name = "tarefas" | "agenda" | "propostas" | "contratos" | "comercial" | "paginas";

const Base = ({ id, from, to, children, size }: { id: string; from: string; to: string; children: React.ReactNode; size: number }) => (
  <svg width={size} height={size} viewBox="0 0 100 100" aria-hidden="true" style={{ display: "block", filter: "drop-shadow(0 4px 8px rgba(0,0,0,.35))" }}>
    <defs>
      ...
      <clipPath id={`clip-${id}`}><path d={SQUIRCLE} /></clipPath>
    </defs>
  ...
  agenda: (s) => (
    <Base id="age" from="#3B3B41" to="#141417" size={s}>
      <g filter="url(#sh-age)">
        ...
        <text x="50" y="74" textAnchor="middle" fontFamily="Inter, system-ui, sans-serif" fontWeight="800" fontSize="30" fill={K}>15</text>
      </g>
    </Base>
  ),
  ...
  contratos: (s) => (
    <Base id="con" from="#2B2B30" to="#0E0E11" size={s}>
      <g filter="url(#sh-con)">
        ...
        <path d="M30 70c6-10 10-10 12-2s5 6 9-3 7-8 10 1" fill="none" stroke={Y} strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" />
        <circle cx="70" cy="70" r="10" fill={Y} />
        <path d="M65 70l3.5 3.5L76 66" fill="none" stroke={K} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
      </g>
    </Base>
  ),
  ...
  comercial: (s) => (
    <Base id="com" from="#FFE04A" to={Y2} size={s}>
      <g filter="url(#sh-com)">
        ...
        <path d="M16 44c0-4 3-7 6.5-7S29 40 29 44Zm13 0c0-4 3-7 6.5-7S42 40 42 44Zm13 0c0-4 3-7 6.5-7S55 40 55 44Zm13 0c0-4 3-7 6.5-7S68 40 68 44Zm13 0c0-4 3-7 6.5-7S84 40 84 44Z" fill="#fff" />
    ...
    <Base id="pag" from="#3B3B41" to="#141417" size={s}>
      <g filter="url(#sh-pag)">
        ...
        {Array.from({ length: 12 }).map((_, i) => (
          <rect key={i} x="49" y="20" width="2" height={i % 3 === 0 ? 7 : 4} rx="1" fill="#8E8E93" transform={`rotate(${i * 30} 50 50)`} />
        ))}
...
export function ArkAppIcon({ name, size = 48, style }: { name: Name; size?: number; style?: CSSProperties }) {
  return <span style={{ display: "inline-block", lineHeight: 0, ...style }}>{icons[name](size)}</span>;
}
