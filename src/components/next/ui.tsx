...
export function Card({
  title, icon, count, action, children, className = "", solid = false, id,
}: {
  title?: string; icon?: ReactNode; count?: number | string; action?: ReactNode;
  children: ReactNode; className?: string; solid?: boolean; id?: string;
}) {
  return (
...
export function Kpi({
  label, value, unit, detail, tone, onOpen, title,
}: {
  label: string; value: number | string; unit?: string; detail?: string;
  tone?: "hot" | "warn" | "ok"; onOpen?: () => void; title?: string;
}) {
  return (
...
export function Abrir({ onClick, children = "Abrir" }: { onClick: () => void; children?: ReactNode }) {
  return (
...
export function TarefaLinha({
  t, cliente, onConcluir, onAbrir, mostrarResp,
}: {
  t: Tarefa; cliente?: string; onConcluir?: (t: Tarefa) => Promise<void>;
  onAbrir?: () => void; mostrarResp?: boolean;
}) {
  ...
  return (
    <div className={`nx-row ${onAbrir ? "clk" : ""}`} onClick={onAbrir}>
      {podeConcluir ? (
        <button
          ...
          onClick={async (e) => {
            e.stopPropagation();
            setBusy(true);
            try { await onConcluir!(t); } finally { setBusy(false); }
          }}
        >
      ...
      <span className="t">
        ...
        <span>
          {[cliente, mostrarResp ? t.resp : null, STATUS_LABEL[t.status || ""]].filter(Boolean).join(" · ")}
        </span>
      </span>
...
export function Skeleton({ n = 4 }: { n?: number }) {
  return (
    <div className="nx-grid" aria-busy="true" aria-label="Carregando">
      {Array.from({ length: n }).map((_, i) => <div key={i} className="nx-skel c3" />)}
